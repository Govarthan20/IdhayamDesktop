...
const REPORT_GRID: React.CSSProperties = {
  display: 'grid',
  gridTemplateColumns: 'minmax(160px, 2fr) 72px 88px 56px 56px 56px 56px',
  columnGap: 16,
  alignItems: 'center',
  padding: '14px 24px 14px 16px',
};

const PER_BOX_CELL: React.CSSProperties = {
  textAlign: 'left',
  marginLeft: -100,
};
...
const ORD_CNF_CELL: React.CSSProperties = {
  textAlign: 'left',
  marginLeft: -90,
};

const qtyCell = (isHeader = false): React.CSSProperties => ({
...
const OrderEntryReportScreen: React.FC = () => {
  ...
  return (
    <div style={{ height: '100%', backgroundColor: '#E2E8F0', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px 36px 40px' }}>
        ...
        {groupedOrders.length > 0 && (
          <div style={{ backgroundColor: '#fff', borderRadius: 20, overflow: 'hidden', boxShadow: '0 4px 15px rgba(0,0,0,0.06)' }}>
            {/* Table header */}
            <div style={{ ...REPORT_GRID, backgroundColor: '#3861FB', padding: '16px 24px 16px 16px' }}>
              <span style={{ fontSize: 13, fontWeight: 900, color: '#fff', textAlign: 'left' }}>ITEM</span>
              <span style={{ ...qtyCell(true), ...PER_BOX_CELL, color: '#fff' }}>PER{'\n'}BOX</span>
              <span style={{ ...qtyCell(true), ...PRICE_CELL, color: '#fff' }}>PRICE</span>
              {['ORD\nBOX', 'ORD\nPCS', 'CNF\nBOX', 'CNF\nPCS'].map((h, i) => (
                <span key={i} style={{ ...qtyCell(true), ...ORD_CNF_CELL, color: '#fff' }}>{h}</span>
              ))}
            </div>
                ...
                <div key={gIdx}>
                  <div onClick={() => setExpandedOrders(prev => { const n = new Set(prev); n.has(group.orderId) ? n.delete(group.orderId) : n.add(group.orderId); return n; })}
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <p style={{ fontSize: 13, color: '#475569', fontWeight: 700, margin: '3px 0 0' }}>{group.date} • <span style={{ color: '#059669', fontWeight: 900 }}>₹ {formatIndianNumber(group.amount || 0)}</span></p>
                    </div>
                  ...
                  {expanded && group.items.map((item: any, iIdx: number) => (
                    <div key={iIdx} style={{ ...REPORT_GRID, borderBottom: '1px solid #E2E8F0', backgroundColor: iIdx % 2 === 0 ? '#FAFBFF' : '#fff' }}>
                      <span style={{ fontSize: 13, fontWeight: 700, color: '#4A5568', textAlign: 'left' }}>{item.itemName || '—'}</span>
                      <span style={{ fontSize: 13, fontWeight: 700, color: '#4A5568', ...PER_BOX_CELL }}>{item.perBox ?? '—'}</span>
                      <span style={{ fontSize: 13, fontWeight: 800, color: '#059669', ...PRICE_CELL }}>{parseFloat(item.price || 0).toFixed(2)}</span>
                      {[item.ordBox, item.ordPcs, item.cnfBox, item.cnfPcs].map((val, vi) => (
                        <span key={vi} style={{ fontSize: 13, fontWeight: 700, color: '#4A5568', ...ORD_CNF_CELL }}>{val ?? '—'}</span>
                      ))}
                    </div>
                  ))}
                </div>
      ...
      {groupedOrders.length > 0 && (
        <div style={{ flexShrink: 0, backgroundColor: '#fff', padding: '12px 36px 20px', borderTop: '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', boxShadow: '0 -4px 20px rgba(0,0,0,0.06)' }}>
          <div>
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <p style={{ fontSize: 20, fontWeight: 900, color: '#3861FB', margin: '4px 0 0' }}>₹{formatIndianNumber(totalAmount)}</p>
          </div>
        </div>
      )}
    </div>
